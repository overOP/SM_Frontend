...
const StudentAnnouncementData = () => {
  ...
  const visible = useMemo(() => {
    const relevant = announcements.filter(
      (a) => a.audience === "Students" || a.audience === "Everyone"
    );
  ...
  return (
    <div className="p-6 bg-slate-50 min-h-screen font-sans text-slate-900">
      ...
      {pinned.length > 0 && (
        <div className="mb-10">
          <div className="flex items-center gap-2 mb-4 text-blue-600 font-black text-sm uppercase tracking-widest">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {pinned.map((item) => (
              <div key={item.id} className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm hover:border-blue-300 transition-colors">
                ...
                <p className="text-sm text-slate-500 line-clamp-2 mb-6">{item.content}</p>
          ...
          <div className="flex flex-col lg:flex-row gap-6">
            {recent.map((item) => (
              <div key={item.id} className="flex-1 bg-white border border-slate-200 rounded-2xl p-5 shadow-sm hover:border-blue-200 transition-colors">
                ...
                <p className="text-xs text-slate-500 leading-relaxed line-clamp-2 mb-5">{item.content}</p>
