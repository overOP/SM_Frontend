...
const StudentData = () => {
  const stats = [
    { label: "Days Present", value: "182", icon: CheckCircle, color: "text-emerald-600", bg: "bg-emerald-50" },
    { label: "Days Absent", value: "08", icon: XCircle, color: "text-rose-600", bg: "bg-rose-50" },
    { label: "Homework Pending", value: "03", icon: BookOpen, color: "text-orange-600", bg: "bg-orange-50" },
    { label: "Current GPA", value: "3.85", icon: GraduationCap, color: "text-blue-600", bg: "bg-blue-50" },
  ];
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {stats.map((stat, i) => (
          <div key={i} className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm flex items-center gap-4 hover:shadow-md transition-shadow">
            <div className={`p-4 rounded-xl ${stat.bg}`}><stat.icon className={stat.color} size={24} /></div>
            <div>
      ...
      {/* You can add a 'Recent Activity' or 'Next Class' section here */}
    </div>
  );
};
