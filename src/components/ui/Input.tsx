...
interface InputProps extends React.InputHTMLAttributes<HTMLInputElement> {
  label?: string;
  error?: string;
  // We define the icon prop as a LucideIcon component
  icon?: LucideIcon; 
}
...
export const Input = ({ 
  label, 
  error, 
  icon: Icon, // Destructure and rename to 'Icon' (capitalized) for JSX
  className = '', 
  ...props 
}: InputProps) => {
  return (
    <div className={`flex flex-col gap-1.5 w-full ${className}`}>
      ...
      <div className="relative">
        ...
        <input
          ...
          `}
          {...props} // Now 'icon' is NOT in here, so the HTML input is happy
        />
      </div>
